...
import { Building2, Loader2, Mail, Lock, Eye, EyeOff, ArrowRight, Home } from 'lucide-react';
...
function LoginPageInner() {
  ...
  const { mutate: login, isPending } = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (user.role !== 'TENANT') {
        toast({ title: 'Access Denied', description: 'This portal is for tenants only.', variant: 'destructive' });
        return;
      }
        ...
        <div className="w-full max-w-sm mx-auto">
          ...
          <div className="rounded-2xl p-7"
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-400 mt-1">Sign in to your tenant dashboard</p>
            </div>
            ...
            <p className="mt-6 text-center text-sm text-slate-500">
              Are you a landlord?{' '}
              <a href={process.env.NEXT_PUBLIC_LANDLORD_APP_URL ?? 'http://localhost:3002'} className="text-indigo-400 hover:text-indigo-300 font-medium transition-colors">
                Landlord portal →
              </a>
            </p>
          </div>
